...
export function MealOnboarding() {
  ...
  return <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
    <div className="space-y-8">
      ...
      <Question title="4. Any major allergens we should filter from the displayed recipes?" optional>
      ...
      <Question title="5. How do you want to receive the meals?">
    ...
    <aside className="h-fit rounded-[1.7rem] bg-[var(--leaf-deep)] p-6 text-white lg:sticky lg:top-6">
      ...
      <dl className="mt-6 space-y-3 border-t border-white/15 pt-5 text-sm"><Row label="Fulfillment" value={fulfillmentMethod} /><Row label="Displayed menu options" value={`${eligible.length}`} /><Row label="Listed allergen filters" value={allergens.length ? `${allergens.length}` : "None"} /></dl>
